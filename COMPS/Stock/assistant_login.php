...
<?php
	...
	if (isset($_POST['login']))
	{
		...
		if ($submit)
		{
			...
			$query = "SELECT * FROM kjscomp_stock.assistant_login WHERE user_id='$user_id' AND password='$password'";
			$result = mysql_query($query) or die (mysql_error());
			$row = mysql_fetch_assoc($result);
			if ( ($user_id == $row['user_id']) && ($password == $row['password']) )
			{
				$_SESSION['assistant'] = $row['name'];
...
<html>
	...
	<body>
		...
		<div>
			...
			<div id="middle">
				<center>
					<form method="POST" action="assistant_login">
						<table>
							<tr>
								<th>
