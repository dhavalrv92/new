...
<?php
	...
	$dataApp = mysql_query("SELECT COUNT(*) AS TotalApp FROM sem_viii") or die(mysql_error());
	$countdataApp = mysql_fetch_assoc($dataApp);
	echo ' <td> '. $countdataApp['TotalApp'] . ' </td> ';
	
	$dataPass = mysql_query("SELECT COUNT(*) AS TotalPass FROM sem_viii WHERE percentage > 40") or die(mysql_error());
	...
	$data60 = mysql_query("SELECT COUNT(*) AS Above60 FROM sem_viii WHERE percentage BETWEEN 60 AND 69") or die(mysql_error());
	$countdata60 = mysql_fetch_assoc($data60);
	echo ' <td colspan=2> ' . $countdata60['Above60'] . '</td> ';
	
	$data70 = mysql_query("SELECT COUNT(*) AS Above70 FROM sem_viii WHERE percentage >= 70") or die(mysql_error());
