...
<?php
	...
	$dataApp = mysql_query("SELECT COUNT(*) AS TotalApp FROM sem_vii") or die(mysql_error());
	$countdataApp = mysql_fetch_assoc($dataApp);
	echo ' <td> '. $countdataApp['TotalApp'] . ' </td> ';
	
	$dataPass = mysql_query("SELECT COUNT(*) AS TotalPass FROM sem_vii WHERE percentage > 40") or die(mysql_error());
